...
CREATE TYPE shops_status AS ENUM('inactive', 'active');
CREATE TABLE shops (
  id UUID PRIMARY KEY,
  name TEXT NOT NULL,
  status shops_status NOT NULL,
...
CREATE TYPE lists_status AS ENUM('draft', 'ready', 'active', 'completed', 'discarded');
CREATE TABLE lists (
  id UUID PRIMARY KEY,
  name TEXT,
  status lists_status NOT NULL,
  shopping_date DATE,
...
CREATE INDEX idx_lists_shop_id ON lists(shop_id);
